...
- **SPA routing on a static host:** this is a single-page app — every route (`/users`, `/shops/manage/:shopId`, etc.) only exists client-side via React Router, not as a real file/path on the deployed static host. Without a fallback, refreshing (or directly opening a link to) any route other than `/` returns a "Not Found" from the host itself, before React Router ever gets a chance to run. `client/public/_redirects` (`/* /index.html 200`) fixes this on Render's Static Sites (and Netlify, which uses the same convention) by rewriting every path to `index.html` — Vite copies `public/` verbatim into `dist/` on build, so this ships automatically with every deploy. A different static host (e.g. Vercel) needs its own equivalent SPA-fallback rewrite instead.
