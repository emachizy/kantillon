...
- **SPA routing on a static host:** this is a single-page app — every route (`/users`, `/shops/manage/:shopId`, etc.) only exists client-side via React Router, not as a real file/path on the deployed static host. Without a fallback rule, refreshing (or directly opening a link to) any route other than `/` returns a "Not Found" from the host itself, before React Router ever gets a chance to run. On **Render Static Sites specifically, this is not fixed by a `_redirects` file** — Render doesn't read that Netlify convention at all (confirmed: it gets served back as a literal static file, not interpreted as a rule). It must be configured as an actual Rewrite rule, either in the Render Dashboard for that static site (**Redirects/Rewrites** tab → Source `/*` → Destination `/index.html` → Action **Rewrite**, not Redirect) or, if this site is managed as a Render Blueprint, via `routes` in `render.yaml`:
  ```yaml
  routes:
    - source: /*
      ...
      type: rewrite
  ```
  A different static host (e.g. Vercel, Netlify) needs its own equivalent SPA-fallback rewrite instead — the mechanism is host-specific.
